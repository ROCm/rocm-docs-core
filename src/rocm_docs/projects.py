...
class _Project:
    ...
    def from_yaml_entry(cls, entry: ProjectEntry) -> _Project:
        ...
        if not isinstance(valid_versions, list):
            valid_versions = [valid_versions] if valid_versions is not None else []
    ...
    def _find_best_valid_version(self, target_version: str) -> str:
        ...
        if regex.match(target_version):
            doc_versions = [version for version in self.valid_versions if version and regex.match(version)]
            if doc_versions:
                try:
                    def calc_version(version_str: str) -> int:
                        version_nums = tuple(map(int, version_str.replace("docs-", "").split(".")))
                        major, minor, patch = version_nums
                        return major * 100 + minor * 10 + patch

                    target_version_value = calc_version(target_version)
                    valid_versions = [(calc_version(v), v) for v in doc_versions]
